...
from tools.spine_side_by_side_report import build_summary, main, write_reports
...
def _create_sample_db(path: Path) -> None:
    ...
    try:
        ...
        conn.execute(
            """
            INSERT INTO guardian_spine_project_events VALUES
            (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            ("pe-1", "project-1", "project.updated", "2026-05-02T00:00:00+00:00", "room-1", "project_lifecycle", "system", "proj-src-1", json.dumps({"status": "active"})),
        )
        conn.commit()
    finally:
        conn.close()
...
def test_missing_db_fails_safely(tmp_path, capsys):
    ...
    code = main(["--source-db", str(tmp_path / "missing.db"), "--output-dir", str(output_dir)])
...
def test_temp_copy_is_used(tmp_path):
    ...
    assert summary["used_temp_copy"] is True
    assert summary["temp_copy_path"] != summary["source_db_path"]


def test_report_files_generated(tmp_path):
    source = tmp_path / "source.db"
    output_dir = tmp_path / "out"
    _create_sample_db(source)
    result = write_reports(source, output_dir)
    assert Path(result["summary_path"]).exists()
    assert Path(result["report_path"]).exists()


def test_report_includes_sparkbot_side(tmp_path):
    source = tmp_path / "source.db"
    _create_sample_db(source)
    summary = build_summary(source)
    assert "sparkbot_side" in summary
    assert "expected_table_count" in summary["sparkbot_side"]


def test_report_includes_lima_side(tmp_path):
    source = tmp_path / "source.db"
    _create_sample_db(source)
    summary = build_summary(source)
    assert "lima_side" in summary
    assert "translation_pass" in summary["lima_side"]


def test_report_includes_pass_fail_and_gap_sections(tmp_path):
    source = tmp_path / "source.db"
    _create_sample_db(source)
    summary = build_summary(source)
    assert "pass_fail" in summary
    assert "known_gaps" in summary
    assert isinstance(summary["known_gaps"], list)
...
def test_no_disallowed_imports():
    mod = importlib.import_module("tools.spine_side_by_side_report")
